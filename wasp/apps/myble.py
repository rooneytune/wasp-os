...
class MyBleApp():
    ...
    def _setupBleService(self):
        ...
        serv_env_sense = Service(uuid_service)
        self._updateBleInfo("Svc Set")       

        global char_led
        char_led = Characteristic(uuid_led, props=Characteristic.PROP_WRITE | Characteristic.PROP_NOTIFY | Characteristic.PROP_READ)
        serv_env_sense.addCharacteristic(char_led)
    ...
    def tick(self, ticks):
        ...
        try:
            ...
            if self._checkboxState == True:
                ...
                if notif_enabled:
                    #char_led.write("hello".encode('utf-8'))
                    char_led.write(bytearray([self._bleUpdateCount & 0xFF, self._bleUpdateCount >> 8]))
        except:
            self._updateBleInfo("Char Err")
...
def event_handler(id, handle, data):
    ...
    try:
        ...
        if id == constants.EVT_GAP_CONNECTED:
            ...
            if int(data[0])==1:
                bleEventMessage="Data:1"
                notif_enabled = True
            else:
                bleEventMessage="Data:{}".format(str(data[0]))
        elif id == constants.EVT_GATTS_READ:
            bleEventMessage="Read"
    except:
        bleEventMessage="{}-Err".format(id)
